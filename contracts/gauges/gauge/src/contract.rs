...
mod execute {
    ...
    pub fn member_changed(
        ...
        for diff in diffs {
                ...
                votes().query_votes_by_voter(deps.as_ref(), &voter, None, Some(query::MAX_LIMIT))?
            {
                ...
                let old = Uint256::new(diff.old.unwrap_or_default().into());
                let new = Uint256::new(diff.new.unwrap_or_default().into());
                ...
                let updates: Vec<_> = vote
                    ...
                    .map(|v| {
                        (
                            v.option.as_str(),
                            old.mul_floor(v.weight),
                            new.mul_floor(v.weight),
                        )
                    })
                    .collect();
        ...
        match msg {
            StakeChangedHookMsg::Stake { addr, amount } => {
                ...
                for mut vote in votes().query_votes_by_voter(
                    ...
                    let new: Uint128 = (vote.power + amount)
                        .try_into()
                        .expect("should never occur");
                    ...
                    let updates: Vec<_> = vote
                        ...
                        .map(|v| {
                            (
                                v.option.as_str(),
                                old.mul_floor(v.weight),
                                new.mul_floor(v.weight).u128().into(),
                            )
                        })
                        .collect();
                    update_tallies(deps.storage, vote.gauge_id, updates)?;
        ...
        match msg {
            NftStakeChangedHookMsg::Stake { addr, token_id: _ } => {
                ...
                for mut vote in votes().query_votes_by_voter(
                    ...
                    let new = vote.power + Uint256::one();
            ...
            NftStakeChangedHookMsg::Unstake { addr, token_ids } => {
                ...
                for mut vote in votes().query_votes_by_voter(
                    ...
                    let new = vote.power - Uint256::new(amount);
    ...
    pub fn place_votes(
        ...
        let mut diff: HashMap<&str, (Uint256, Uint256)> = previous_vote
            .votes
            .iter()
            .map(|v| {
                (
                    v.option.as_str(),
                    (power.mul_floor(v.weight), Uint256::zero()),
                )
            })
            .collect();
        for v in new_votes.iter() {
            let new = voting_power.mul_floor(v.weight);
            let add = match diff.remove(v.option.as_str()) {
                Some((old, _)) => (old, new),
                None => (Uint256::zero(), new),
            };
        ...
        for new_opt in diff
            .iter()
            .filter(|(_, (old, _))| *old == Uint256::zero())
            .map(|(&k, _)| k)
        {
        ...
        let selected = selected_set_with_powers
            ...
            .map(|(option, power)| {
                Ok((
                    option,
                    Decimal::from_ratio(
                        Uint128::try_from(power)
                            .expect("should never overflow u128")
                            .u128(),
                        Uint128::try_from(selected_powers_sum)
                            .expect("should never overflow u128")
                            .u128(),
                    ),
                ))
            ...
            .map(|o| {
                ...
                if let Some(max_available_percentage) = gauge.max_available_percentage {
                    if Decimal::from_ratio(power, total_cast) > max_available_percentage {
                        ...
                        return Ok((
                            option,
                            Uint256::new(total_cast).mul_floor(max_available_percentage),
                        ));
                    }
                }
                Ok((option, Uint256::new(power)))
...
pub fn migrate(deps: DepsMut, env: Env, msg: MigrateMsg,info: MigrateInfo) -> Result<Response, ContractError> {
    ...
    for (gauge_id, config) in msg.gauge_config.unwrap_or_default() {
        GAUGES.update(deps.storage, gauge_id, |gauge| -> StdResult<_> {
            let mut gauge = gauge
                .ok_or_else(|| StdError::msg(format!(" not found: Gauge with id {}", gauge_id)))?;
            if let Some(next_epoch) = config.next_epoch {
                if next_epoch < env.block.time.seconds() {
                    return Err(StdError::msg(
                        "Next epoch value cannot be earlier then current epoch!",
                    ));
                }

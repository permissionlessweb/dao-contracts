...
fn add_option() {
    ...
    let mut suite = SuiteBuilder::new()
        .with_voting_members(&[(voter1.as_str(), 100), (voter2.as_str(), 200)])
        .build();
    ...
    suite
        .add_option(&gauge_contract, voter1.as_str(), gauge_id, "addedoption1")
        .unwrap();
    suite
        .add_option(&gauge_contract, voter2.as_str(), gauge_id, "addedoption2")
        .unwrap();
    let options = suite.query_list_options(&gauge_contract, gauge_id).unwrap();
...
fn vote_for_option() {
    ...
    suite
        .place_votes(
            ...
            voter1.clone(),
            gauge_id,
            Some(vec![(voter1.clone(), Decimal::percent(90))]),
        )
        .unwrap();
    assert_eq!(
    ...
    assert_eq!(selected_set, vec![(voter1.clone(), Uint256::new(90))]);
    ...
    suite
        .add_option(&gauge_contract, voter1.as_str(), gauge_id, "option1")
        .unwrap();
    suite
        .add_option(&gauge_contract, voter1.as_str(), gauge_id, "option2")
        .unwrap();
...
fn vote_for_max_capped_option() {
    ...
    assert_eq!(
        selected_set,
        vec![
            ("option1".to_owned(), Uint256::new(11)),
            ("option2".to_owned(), Uint256::new(10))
        ]
    );
}
...
fn membership_voting_power_change() {
    ...
    suite
        .propose_add_membership_change_hook(voter1.clone(), gauge_contract.clone())
        .unwrap();
    ...
    assert_eq!(
        ...
        vec![
            ("option2".to_string(), Uint256::new(100)),
            ("option1".to_string(), Uint256::new(100)),
            (voter1.clone(), Uint256::new(90))
        ]
    );
    ...
    suite
        .force_update_members(vec![voter1.clone()], vec![])
        .unwrap();
    suite.next_block();
...
fn token_staking_voting_power_change() {
    ...
    let voter1 = api.addr_make("voter1").to_string();
    let voter2 = api.addr_make("voter2").to_string();
    let hook_caller = api.addr_make("token-staking-contract").to_string();
    let mut suite = SuiteBuilder::new()
        .with_voting_members(&[(voter1.as_str(), 100), (voter2.as_str(), 200)])
        .with_core_balance((10000, "ujuno"))
        .build();
    ...
    suite
        .app
        .execute_contract(
            Addr::unchecked(hook_caller.as_str()),
            gauge_contract.clone(),
            &StakeChangedExecuteMsg::StakeChangeHook(
                dao_hooks::stake::StakeChangedHookMsg::Stake {
                    addr: Addr::unchecked(voter1.as_str()),
                    amount: Uint256::new(900),
                },
            ),
            &[],
        )
...
fn nft_staking_voting_power_change() {
    ...
    let voter1 = api.addr_make("voter1").to_string();
    let voter2 = api.addr_make("voter2").to_string();
    let hook_caller = api.addr_make("nft-staking-contract").to_string();
    let mut suite = SuiteBuilder::new()
        .with_voting_members(&[(voter1.as_str(), 1), (voter2.as_str(), 2)])
        .with_core_balance((10000, "ujuno"))
        .build();
    ...
    suite
        .place_votes(
            ...
            voter1.clone(),
            gauge_id,
            Some(vec![(voter1.clone(), Decimal::percent(100))]),
        )
        .unwrap();
    assert_eq!(
        simple_vote(&voter1, &voter1, 100, suite.current_time()),
    ...
    // check tally is proper
    let selected_set = suite.query_selected_set(&gauge_contract, gauge_id).unwrap();
    assert_eq!(selected_set, vec![(voter1.clone(), Uint256::one())]);

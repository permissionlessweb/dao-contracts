...
impl Suite {
    ...
    pub fn query_last_executed_set(
        ...
        gauge_contract: &Addr,
        id: u64,
    ) -> StdResult<Option<Vec<(String, Uint256)>>> {
    ...
    pub fn query_list_options(
        ...
        options.sort_by_key(|(k, _)| k.clone());
        Ok(options)
    }
    ...
    pub fn query_list_votes(&self, gauge_contract: &Addr, id: u64) -> StdResult<Vec<VoteInfo>> {
        ...
        votes.sort_by_key(|v| v.voter.clone());
        Ok(votes)
    }
    ...
    pub fn find_gauge_module(&self) -> Addr {
        let proposal_modules = self.query_proposal_modules().unwrap();
        proposal_modules
            .into_iter()
            .find(|addr| *addr != self.proposal_single)

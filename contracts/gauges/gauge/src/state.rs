...
use cosmwasm_std::{Addr, Decimal, Deps, Env, Order, StdResult, Storage, Uint128, Uint256};
...
pub struct Gauge {
    ...
    pub last_executed_set: Option<Vec<(String, Uint256)>>,
...
struct VoteIndexes {
    ...
    pub vote: MultiIndex<'static, GaugeId, WeightedVotes, (Addr, GaugeId)>,
}

impl IndexList<WeightedVotes> for VoteIndexes {
...
pub struct Votes {
    ...
    votes: IndexedMap<(Addr, GaugeId), WeightedVotes, VoteIndexes>,
}

impl Votes {
    pub fn new(storage_key: &'static str, vote_subkey: &'static str) -> Self {
    ...
    pub fn save(
        ...
        self.votes.save(storage, (voter.clone(), gauge_id), vote)
    }
    ...
    pub fn set_votes(
        ...
        gauge_id: GaugeId,
        votes: Vec<Vote>,
        power: impl Into<Uint256>,
    ) -> StdResult<()> {
        let power = power.into();
        self.votes.save(
            storage,
            (voter.clone(), gauge_id),
            &WeightedVotes {
                gauge_id,
                power,
                votes,
    ...
    pub fn remove_votes(
        ...
        self.votes.remove(storage, (voter.clone(), gauge_id))
    }
    ...
    pub fn load(
        ...
        self.votes.load(storage, (voter.clone(), gauge_id))
    }
    ...
    pub fn may_load(
        ...
        self.votes.may_load(storage, (voter.clone(), gauge_id))
    }
    ...
    pub fn query_votes_by_gauge(
        ...
        let start = addr
            .as_ref()
            .map(|a| Bound::exclusive((a.clone(), gauge_id)));
        ...
        self.votes
            ...
            .range(deps.storage, start, None, Order::Ascending)
            .take(limit)
            .filter(|r: &Result<((Addr, GaugeId), WeightedVotes), _>| match r {
                Ok((_, v)) => !v.is_expired(&gauge), // filter out expired votes
                Err(_) => true,                      // keep the error
            })
            .map(|r| {
                let ((voter, _gauge), votes): ((Addr, GaugeId), WeightedVotes) = r?;
                Ok(VoteInfo {
...
pub fn update_tally(
    ...
    update_tallies(
        storage,
        gauge,
        vec![(option, old_vote.into(), new_vote.into())],
    )
}
...
pub fn update_tallies(
    ...
    gauge: GaugeId,
    // (option, old, new)
    updates: Vec<(&str, Uint256, Uint256)>,
) -> StdResult<()> {
    let mut old_votes = Uint256::zero();
    let mut new_votes = Uint256::zero();
    ...
    for (option, old_vote, new_vote) in updates {
        ...
        let count = Uint256::new(old_count.unwrap_or_default()) + new_vote - old_vote;

        // update main index
        TALLY.save(
            storage,
            (gauge, option),
            &Uint128::try_from(count)
                .expect("should never overflow u128")
                .u128(),
        )?;
    ...
    // update total count
    let total = Uint256::new(TOTAL_CAST.may_load(storage, gauge)?.unwrap_or_default());
...
mod tests {
    ...
    fn votes_works() {
            ...
            GAUGES
                .save(
                    ...
                    &Gauge {
                        ...
                        adapter: api.addr_make("gauge_adapter"),
                        epoch: 100,
        ...
        votes
            .votes
            .save(&mut deps.storage, (user1.clone(), 1), &vote1)
            .unwrap();
        ...
        votes
            .votes
            .save(&mut deps.storage, (user2.clone(), 1), &vote2)
            .unwrap();
        ...
        votes
            .votes
            .save(&mut deps.storage, (user3.clone(), 1), &vote3)
            .unwrap();
        ...
        let vote4 = WeightedVotes {
            gauge_id: 2,
            power: Uint256::new(12),
            votes: votes4,
            cast: Some(env.block.time.seconds()),
        };
        votes
            .votes
            .save(&mut deps.storage, (user1.clone(), 2), &vote4)
            .unwrap();
        ...
        let vote5 = WeightedVotes {
            gauge_id: 3,
            power: Uint256::new(15),
            votes: votes5,
            cast: Some(env.block.time.seconds()),
        };
        votes
            .votes
            .save(&mut deps.storage, (user1.clone(), 3), &vote5)
            .unwrap();

        let vote_result = votes.votes.load(&deps.storage, (user2.clone(), 1)).unwrap();
    ...
    fn query_votes_by_user_paginated() {
        ...
        votes
            .votes
            .save(&mut deps.storage, (user1.clone(), 2), &vote1)
            .unwrap();
        ...
        votes
            .votes
            .save(&mut deps.storage, (user1.clone(), 3), &vote2)
            .unwrap();
        ...
        votes
            .votes
            .save(&mut deps.storage, (user1.clone(), 4), &vote3)
            .unwrap();
